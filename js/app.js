...
export async function bootstrapGEF() {
  ...
  try {
    // 1. Inicializar infraestrutura de PWA e suporte offline
    initPWA();

    // 2. Inicializar cliente Supabase com credenciais salvas
    ...
    // 4. Sincronizar sessão de autenticação ativa e RBAC
    try {
      await syncSession();
    } catch (syncErr) {
      console.warn('[GEF] Aviso na sincronização de sessão:', syncErr);
    }

    // 5. Inicializar roteador
    initRouter();
  } catch (err) {
    console.error('[GEF] Erro fatal na inicialização:', err);
    try {
      setupShell();
      initRouter();
    } catch {
